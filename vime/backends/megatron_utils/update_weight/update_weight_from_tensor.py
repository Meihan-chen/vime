...
``--worker-extension-cls``; selected by the platform provider.
...
def _build_ipc_update_info_from_named_tensors(
    ...
    """Build vLLM IPC ``update_info`` payload from tensors on this rank's device.

    Each handle is keyed by the physical device UUID of the producing rank
    rather than by a local device index. The coordinator gathers all ranks'
    dicts and merges them; the native receiver looks up its own UUID and
    remaps the handle to its local device index.

    Return the contiguous tensor refs alongside the payload. ``reduce_tensor``
    only exports IPC metadata, so the producer storage must stay alive until
    the receiver opens the handle.
    """
...
class UpdateWeightFromTensor:
    ...
    def update_weights(self) -> None:
        ...
        for hf_named_tensors in self._hf_weight_iterator.get_hf_weight_chunks(megatron_local_weights):
            ...
            ray.get(refs)
            # Keep every producer's storage alive until the slot leader's
            # native update request has consumed all IPC handles.
...
# Compatibility aliases for the old ``--worker-extension-cls`` paths.
from .npu_worker_extension import vLLMColocateWorkerExtension, vLLMWorkerExtension  # noqa: E402,F401

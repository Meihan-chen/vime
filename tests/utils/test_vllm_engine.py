...
@pytest.mark.unit
def test_build_vllm_subprocess_env_delegates_visible_devices_to_platform(vllm_args, monkeypatch):
    seen = {}

    def subprocess_env(env, *, visible_devices, colocate):
        seen.update(visible_devices=visible_devices, colocate=colocate)
        assert env["CUDA_VISIBLE_DEVICES"] == visible_devices
        assert env["HIP_VISIBLE_DEVICES"] == visible_devices
        return {**env, "PLATFORM_ENV": "1"}

    platform = SimpleNamespace(vllm=SimpleNamespace(subprocess_env=subprocess_env))
    monkeypatch.setattr(mod, "current_platform", lambda: platform)
    vllm_args.colocate = True

    env = mod._build_subprocess_env({"_args": vllm_args, "_visible_devices": "2,3"})

    assert seen == {"visible_devices": "2,3", "colocate": True}
    assert env["PLATFORM_ENV"] == "1"
